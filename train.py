...
class LaserTracker(object):
    def __init__(self, cam_width, cam_height, hue_min=105, hue_max=255,sat_min=0, sat_max=255, val_min=230, val_max=255):
    ...
    def track(self, frame, mask):
        ...
        if len(countours) > 0:
            ...
            if 2<radius <30:
                ...
                # c_x = (x + 330.0)*8.0/
                # c_y = (y - 220.0) * 10.5 / 640.0
                # frame = frame[40:400, 147:513]
                # cv2.putText(frame,"".join(["Target is at "+str(round(c_x,2))+","+str(round(c_y,2))]), (20,20), cv2.FONT_HERSHEY_SIMPLEX,.5,(0,0,255))
    ...
    def detect(self, frame):
        hsv_img = cv2.GaussianBlur(frame, (5, 5), 20)
        ...
        #     self.channels['value'],
        # ])
        # cv2.imshow("tt", self.channels['laser'])
        lower_red = np.array([self.hue_min, self.sat_min, self.val_min])
        upper_red = np.array([self.hue_max, self.sat_max, self.val_max])
        mask = cv2.inRange(hsv_img, lower_red, upper_red)
        center = self.track(frame, mask)
